...
void loop() {
  if (rfid.isCard()) {
    if (rfid.readCardSerial()) {
      ...
      entrystatus(); // if there is an entry in the database already, we know if its an exiting or entering tag
...
void entrystatus() {
  for (int i = 0; i < tags_in_circulation; i++) {
    if (detected_card.UID == database[i].UID) {
      detected_card.entry = database[i].entry;
    }
  }
  detected_card.entry = !detected_card.entry;
}
...
void printcard() {
  ...
  Serial.print(" - ");
  if (detected_card.entry == true) { Serial.print("entry"); }
  if (detected_card.entry == false) { Serial.print("exit"); }
